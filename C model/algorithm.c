/**
 * @file algorithm.c
 * @brief Core control logic for the drone navigation system.
 *
 * This file implements the main navigation algorithm. It uses GPS data to
 * calculate the bearing and distance to a target, then computes pitch and
 * roll commands using a non-linear control law. All trigonometric calculations
 * are performed using the fast, integer-based CORDIC module.
 */
#include "algorithm.h"
#include "cordic.h" // Use our CORDIC math library
#include <math.h>   // For fmod, sqrt, atan, tanh
...
double haversine_distance(GPSCoordinate_t pos1, GPSCoordinate_t pos2) {
    ...
    // Clamp 'a' to prevent sqrt of a negative number due to floating point errors
    a = clamp(a, 0.0, 1.0);

    // Use fast_atan2 instead of atan2(sqrt(a), sqrt(1-a))
...
static double calculate_bearing(GPSCoordinate_t pos1, GPSCoordinate_t pos2) { //TODO: verify logic with
    ...
    double theta = fast_atan2(y, x); // Use CORDIC for efficiency
...
bool navigator_update(DroneNavigator_t *nav, GPSCoordinate_t current_pos,
    ...
    double error_forward = distance * cos_arg; // cos(relative_bearing)
    double error_right = distance * sin_arg;   // sin(relative_bearing)

    // --- P-Term ---
    // Using standard math library atan for simplicity, can be swapped for CORDIC
    double pitch_p_term = (atan(nav->p_gain * error_forward) / (M_PI / 2.0)) * nav->max_pitch_cmd;
    double roll_p_term = (atan(nav->p_gain * error_right) / (M_PI / 2.0)) * nav->max_roll_cmd;
